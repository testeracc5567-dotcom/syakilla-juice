...
// Struktur: { [productId]: [{ id, name, email, stars, text, ts, orderId }] }
// 1 ulasan per PESANAN, jadi kalau beli produk yang sama 2x, dua-duanya bisa diulas.
import { getCustomerOrders, isOrderCancelled } from "@/lib/orders";
...
// Udah ngulas produk ini DARI pesanan tertentu?
export function hasReviewedOrder(productId, email, orderId) {
  if (!email || !orderId) return false;
  return getReviews(productId).some(
    (r) => r.email === email && String(r.orderId || "") === String(orderId),
  );
}

// Cari pesanan (belum diulas) yang bisa dipakai buat ngulas produk ini.
// Balikin "" kalau semua pesanan yang ada produk ini udah diulas.
export function pendingReviewOrderId(productId, email) {
  if (!productId || !email) return "";
  let list = [];
  try {
    list = getCustomerOrders(email).orders || [];
  } catch (e) {
    return "";
  }
  for (let i = 0; i < list.length; i++) {
    const o = list[i] || {};
    if (isOrderCancelled(o.status)) continue;
    const punya = (o.items || []).some(
      (it) => String(it.id) === String(productId),
    );
    if (!punya) continue;
    if (!hasReviewedOrder(productId, email, o.id)) return String(o.id);
  }
  return "";
}
